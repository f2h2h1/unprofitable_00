...
function top2(){
   	form3.action="<%=basePath%>admin/sc/s2info.jsp?page=1";
    form3.submit();
}
function last2(){
	...
	form3.action="<%=basePath%>admin/sc/s2info.jsp?page="+form3.pageCount.value;
    	form3.submit();
	}
}
function pre2(){
  ...
  if(page<=1){
    ...
    form3.action="<%=basePath%>admin/sc/s2info.jsp?page="+(page-1);
    form3.submit();
  }
}
...
function bjump2(){
	...
	if(form3.busjump.value>pageCount){//如果跳转文本框中输入的页数超过最后一页的数，则跳到最后一页
	  ...
	  form3.submit();
		...
		form3.action="<%=basePath%>admin/sc/s2info.jsp?page="+pageCount;
		form3.submit();
	}
}
...
var page=parseInt(form3.busjump.value);
   if(page==0){
      ...
      form3.action="<%=basePath%>admin/sc/s2info.jsp?page="+page;
      form3.submit();
   }else{
      form3.action="<%=basePath%>admin/sc/s2info.jsp?page="+page;
      form3.submit();
...
<div class="title_right"><strong>上交作业管理</strong></div>  
...
<table class="table table-bordered table-striped table-hover">
     <tbody>
       <tr align="center">
         ...
         <td nowrap="nowrap"><strong>上交作业名称</strong></td> 
         <td nowrap="nowrap"><strong>下载</strong></td> 
         <td nowrap="nowrap"><strong>作业说明</strong></td> 
...
<%String word=Common.toChineseAndTrim(request.getParameter("word")); 
	...
	int cou = cb.getMessageCount("select count(*) from sc where mc like '%"+word+"%' and js='"+username+"'");//得到信息总数			        
	...
	List pagelist1 = cb.getMessage(Integer.parseInt(page1),"select * from sc where mc like '%"+word+"%' and js='"+username+"' order by id desc",8);
...
%>       
       <tr align="center">
         ...
         <td nowrap="nowrap">
         <a href="<%=basePath%>admin/sc/sp.jsp?method=spsc&id=<%=pagelist2.get(0).toString()%>">批阅</a>
         <a href="<%=basePath%>ComServlet?method=delsc2&id=<%=pagelist2.get(0).toString()%>">删除</a></td>  
       </tr>
<% }} %>
       <tr align="center">
