...
<div class="title_right"><strong>商品订购管理</strong></div>  
...
%>
<label>商品编号：<input type="text" name="goods_id" value="<%=goods_id%>"></label>
<label>商品名称：<input type="text" name="goods_name" value="<%=goods_name%>"></label>
<label>商品分类：<input type="text" name="goods_class" value="<%=goods_class%>"></label>
...
<table class="table table-bordered table-striped table-hover">
     <tbody>
       <tr align="center">
         ...
         <td nowrap="nowrap"><strong>商品编号</strong></td>   
         <td nowrap="nowrap"><strong>商品名称</strong></td>
         <td nowrap="nowrap"><strong>商品分类</strong></td>   
...
String sql1;
	...
	if(!goods_id.equals("")){
		count++;
		queue.offer(" id = " + goods_id + " ");
	}
	if(!goods_name.equals("")){
		count++;
		queue.offer(" goods_name like '%" + goods_name + "%' ");
	}
	if(!goods_class.equals("")){
		count++;
		queue.offer(" goods_class like '%" + goods_class + "%' ");
	}
	String where = "";
	if (count > 0) {
		where = " where ";
	...
	List pagelist1 = cb.getMessage(Integer.parseInt(page1),sql2,4);
...
%>       
       <tr align="center">
         ...
         <td nowrap="nowrap">
         <a href="<%=basePath%>admin/cw/addzf.jsp?method=upzf&id=<%=pagelist2.get(0).toString()%>">修改</a> 
         <a href="<%=basePath%>ComServlet?method=delzf&id=<%=pagelist2.get(0).toString()%>">删除</a>
         </td>
       </tr>
<% }} %>
       <tr align="center">
